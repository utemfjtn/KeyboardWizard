...
7. 点击 **「添加」** 按钮 → 进入机器人**详情页/完成页**
8. **在详情页找到 webhook 地址**：
   - 地址形如 `https://open.feishu.cn/open-apis/bot/v2/hook/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`
   - 通常在页面中部的 **「Webhook 地址」** 区域，旁边有 **「复制」** 按钮
   - **如果没看到地址**：
     - 检查是否在「安全设置」折叠区域下方，需要向下滚动页面
     - 或者关闭当前弹窗，回到「设置 → 群机器人」列表，**点击刚创建的机器人**进入详情页查看
     - 如果详情页也看不到，说明飞书版本较旧或企业策略禁用，参考下面的「排查」
9. 点 **「复制」** 把 webhook 地址保存下来，然后点 **「完成」**
10. 群里会出现「机器人已加入」的提示

> **重要**：webhook 地址只在创建时显示一次，**关闭页面后无法再次查看**。如果丢了，只能在「群机器人」列表里**删除该机器人再重建**。如果地址泄露，立即删除机器人重建。

**排查：详情页真的没有地址？**

- 飞书桌面端 V7.5 以下版本可能不显示，升级到最新版
- 企业版管理员可能禁用了自定义机器人功能，联系管理员开通
- 外部群（跨企业群）不支持自定义机器人 webhook，需用内部群
- 移动端（手机/平板）添加机器人后不显示地址，必须在**桌面端**查看
